...
# 合并三组数据形成最终的test1
test1 = pd.concat([test1_part1], ignore_index=True)
print(len(test1))
# 修改结束
# 获取测试集去掉test0的部分
...
# 保存图形
plt.savefig('补充实验2/data_distribution.png')
...
# 保存新的测试集到CSV文件
new_test_data.to_csv('补充实验2/new_test_dataset.csv', index=False)
...
# 保存test1的新数据集到CSV文件
test1_new_data.to_csv('补充实验2/test1_new_dataset.csv', index=False)
...
# 保存图形
plt.savefig('补充实验2/grid_distribution.png')

# 显示图形
plt.show()

# 创建新文件夹用于存储点及其网格边界信息
points_info_dir = '补充实验2/points_with_grid_info'
