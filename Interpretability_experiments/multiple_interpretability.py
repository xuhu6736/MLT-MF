    ...
    def animate(step):
        ...
        if len(train_points) > 0:
            ...
            if len(train_positive) > 0:
                ax.scatter(train_positive[:, 0], train_positive[:, 1], alpha=0.7, c='#2E7D32', s=40, label='训练集-预测正')
            if len(train_negative) > 0:
                ax.scatter(train_negative[:, 0], train_negative[:, 1], alpha=0.7, c='#C62828', s=40, label='训练集-预测负')
        ...
        if len(test_points) > 0:
            ...
            if len(test_positive) > 0:
                ax.scatter(test_positive[:, 0], test_positive[:, 1], alpha=0.7, c='#43A047', s=40, label='测试集-预测正')
            if len(test_negative) > 0:
                ax.scatter(test_negative[:, 0], test_negative[:, 1], alpha=0.7, c='#E53935', s=40, label='测试集-预测负')
        ...
        if len(valid_random_points) > 0:
            ...
            if len(random_positive) > 0:
                ax.scatter(random_positive[:, 0], random_positive[:, 1], alpha=0.7, c='#1565C0', s=40, label='随机点-预测正')
            if len(random_negative) > 0:
                ax.scatter(random_negative[:, 0], random_negative[:, 1], alpha=0.7, c='#FFA000', s=40, label='随机点-预测负')
        ...
        ax.set_xlabel('x1')
        ax.set_ylabel('x2')
        ax.set_title(f'数据集和随机点预测结果可视化 (第{step}步检查点)')
        ax.legend()
...
def visualize_model_parameters():
    ...
    with open(save_path, 'w', encoding='utf-8') as f:
        f.write(f'模型参数详细信息 - 检查点 step {checkpoint_step}\n')
        f.write('=' * 80 + '\n\n')
        
        for name, param_values in zip(param_names, params):
            # 写入参数信息
            f.write(f'参数名称: {name}\n')
            f.write(f'参数形状: {param_values.shape}\n')
            f.write(f'参数数量: {param_values.size}\n')
            f.write(f'参数值:\n{param_values}\n')
            f.write('-' * 80 + '\n\n')
    
    print(f'模型参数详细信息已保存到: {save_path}')
...
def plot_error_vs_steps():
    ...
    # 绘制预测值曲线
    plt.plot(step_numbers, pred_values_1_y1, 'b-', linewidth=2, label='训练集内(1,1)点的y1预测值')
    plt.plot(step_numbers, pred_values_1_y2, 'g-', linewidth=2, label='训练集内(1,1)点的y2预测值')
    plt.plot(step_numbers, pred_values_2_y1, 'r-', linewidth=2, label='测试集内(-1.3535,-1.3535)点的y1预测值')
    plt.plot(step_numbers, pred_values_2_y2, 'm-', linewidth=2, label='测试集内(-1.3535,-1.3535)点的y2预测值')
    ...
    # 添加红色虚线y=0
    plt.axhline(y=0, color='red', linestyle='--', linewidth=1.5, label='y=0参考线')

    plt.xlabel('训练步数')
    plt.ylabel('预测值')
    plt.title('两个点的y1和y2预测值随训练步数变化')
    ...
    plt.close()
    print(f'预测值变化曲线图已保存到: {save_path}')
...
if __name__ == '__main__':
    ...
    plot_error_vs_steps()
    print('\n所有功能已完成，结果保存在 ./可解释性实验/result 文件夹中')