...
# 设置标题
ax.set_title('训练数据与有噪声测试数据结构分布图')

# 1. 绘制0.5-1.5, 0.5-1.5的蓝色矩形
blue_rect = Rectangle((0.5, 0.5), 1.0, 1.0, color='blue', alpha=0.5, label='训练数据区域')
...
ax.fill(x_circle[red_mask], y_circle[red_mask], 'red', alpha=0.5, label='测试集区域')

# 4. 绘制x2>1.4的绿色部分
green_mask = y_circle > 1.4
#ax.fill(x_circle[green_mask], y_circle[green_mask], 'white', alpha=0.5, label='排除区域')
...
# 绘制深黑色的灰色部分
ax.fill(x_green_part, y_green_part, '#303030', alpha=0.5, label='叠加噪声区域')
